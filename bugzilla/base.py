...
from .backendxmlrpc import BackendXMLRPC
...
class Bugzilla(object):
    ...
    def __init__(self, backend=None, **kwargs):
        if backend:
            if kwargs:
                raise TypeError("pass either backend or arguments for the XMLRPC backend")
            self.backend = backend
        else:
            self.backend = BackendXMLRPC(**kwargs)
        self._bug_autorefresh = False

        self._field_aliases = []
        self._init_field_aliases()

        if isinstance(self.backend, BackendXMLRPC) and self.backend._transport:
            self._init_class_from_url()
        self._init_class_state()

    def _init_class_from_url(self):
        """
        Detect if we should use RHBugzilla class, and if so, set it.  This only works
        for the XMLRPC interface.
        """
    ...
    def connect(self, url=None):
        if not isinstance(self.backend, BugzillaBackendXMLRPC):
            raise NotImplementedError("Calling connect() is only supported for XMLRPC")
    ...
    def login(self, user=None, password=None, restrict_login=None):
        if not isinstance(self.backend, BugzillaBackendXMLRPC):
            raise NotImplementedError("Calling login() is only supported for XMLRPC")
        self.backend.login(user=user, password=password, restrict_login=restrict_login)

    def interactive_login(self, user=None, password=None, force=False,
                          restrict_login=None, use_api_key=False):
        if not isinstance(self.backend, BugzillaBackendXMLRPC):
            raise NotImplementedError("Calling interactive_login() is only supported for XMLRPC")
        self.backend.interactive_login(user=user, password=password, force=force,
            restrict_login=restrict_login, use_api_key=use_api_key)

    def logout(self):
        if not isinstance(self.backend, BugzillaBackendXMLRPC):
            raise NotImplementedError("Calling logout() is only supported for XMLRPC")
        self.backend.logout()
